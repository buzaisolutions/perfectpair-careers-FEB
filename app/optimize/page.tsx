...
import { Loader2, Sparkles, ArrowRight, TrendingUp, AlertTriangle, CheckCircle, FileText } from 'lucide-react'
...
export default function OptimizePage() {
  ...
  const [jobTitle, setJobTitle] = useState('') // NOVO CAMPO OBRIGATÓRIO
    ...
    try {
      const response = await fetch('/api/optimize', {
        ...
        body: JSON.stringify({
          ...
          jobTitle: jobTitle,       // AGORA ENVIAMOS O TÍTULO
          jobDescription: jobDescription,
          optimizationType: 'RESUME_ONLY' // AGORA ENVIAMOS O TIPO
        })
      })
      ...
      // --- LEITURA DO STREAM (A MÁGICA) ---
      // O Backend manda dados aos poucos (Stream). Precisamos ler pedacinho por pedacinho.
      ...
      if (reader) {
        while (true) {
          ...
          // O Backend manda "data: {json...}"
          // Vamos tentar achar o JSON final quando ele chegar
          ...
          setStreamMessage('AI is analyzing and writing...')
        }
      }

      if (finalJson) {
        // Mapeia o resultado complexo do backend para o formato simples da tela
        setResult({
          originalScore: Math.floor(finalJson.atsScore * 0.4), // Simulação do antes
          optimizedScore: finalJson.atsScore, // Score real calculado
        ...
        {step === 'result' && result && (
          <div className="space-y-8">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <Card>
                <CardHeader><CardTitle className="text-red-600 flex items-center"><AlertTriangle className="mr-2 w-5 h-5"/> Missing Keywords Detected</CardTitle></CardHeader>
                <CardContent>
